...
import time
# copiar directorio a servidor remoto
...
fecha_de_backup = time.strftime("%d_%m_%Y_%H_%M_%S")
...
def file_replace(fname, archivo_backup):
    ...
    archivo_de_la_lista_de_modificaciones = ruta_completa_carpeta_backup  + fecha_de_backup + ".txt"
    ...
    with open(fname) as f:
        ...
        out.write(new_data)
        out.close()
	#os.system("scp " + fname + " texto@192.168.1.3:/home/texto/back_up")
        ...
        if fichero_moderno_size != fichero_antiguo_size:
            lista_archivos_modificados = open(archivo_de_la_lista_de_modificaciones , 'a')
...
def mass_replace(directorio_de_trabajo, replace_extensions=DEFAULT_REPLACE_EXTENSIONS):
    ...
    # se llama al metodo para copiar los archivos de backup al servidor remoto	
    copy()

def copy():
    # se define la ruta y usuario del servidor remoto (con env.hosts NO funciona)
    env.host_string = 'cepal@192.168.1.3'
    
    # creamos el directorio en el servidor remoto
    run('mkdir -p /home/cepal/back_up/' + fecha_de_backup)

    # se copia la carpeta de backup en el servidor remoto
    put(ruta_completa_carpeta_backup, '/home/cepal/back_up/')
